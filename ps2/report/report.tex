...
% (i)
\subsubsection{Plotting the Particle in a Ring}
The code in \verb|1d.py| is a modified version of \verb|1a.py| changed to implement periodic boundary conditions. The modified code produced the probability density and the numerical eigenvalue plots in Figures \ref{fig:1d_probdensity} and \ref{fig:1d_numerical}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/1d_probdensity.png}
    \caption{Position-probability densities from the numerical wave functions $\{\phi\}$ for eigenvalue numbers $1$ and $50$.}
    \label{fig:1d_probdensity}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/1d_numerical.png}
    \caption{The 100 eigenvalues that resulted from the numerical solution.}
    \label{fig:1d_numerical}
\end{figure}

\noindent Running \verb|1d.py| to get the eigenvalue numbers 4 and 5 produced the output:
...
% 2c
\subsection{Solving the Radial Equation}
\noindent The code in \verb|1a.py| was modified to what is seen in \verb|2c.py|. \verb|2c.py| was used to produce the plots for the first two eigenvectors, corresponding to the 1s and 2s energy states, as seen in Figures \ref{fig:2f_eigenvalue1} and \ref{fig:2f_eigenvalue2}. \\

% 2d
\noindent \verb|1c.py| also produced the following output for the numerical value of the eigenvalue (or eigenenergy) for the 1st level
\begin{verbatim}
Eigenvalue 1: -13.497781923462465 eV
\end{verbatim}

% 2e
\subsection{Finding the Analytical Result}
\noindent The analytical expression for the 1s eigenfunction is
\begin{equation}\label{eq:2e_analytical}
    f(r) = \frac{2r}{a_0^{3/2}} e^{-r/a_0}
\end{equation}
where $a_0 = 4\pi\epsilon_0\hbar^2/mq^2 = \qty{0.0529}{\nano\meter}$ is known as the \textit{Bohr radius}.\\

\noindent Given the radial Schrödinger equation for $l_o = 0$
\[
    -\frac{\hbar^2}{2m} \frac{d^2f}{dr^2} - \frac{q^2}{4\pi\epsilon_0 r} f(r) = E f(r)
\]
\noindent And the 1s eigenfunction
\[
    f(r) = C r e^{-r/a_0} \quad \text{where} \quad a_0 = \frac{4\pi\epsilon_0\hbar^2}{mq^2}
\]
\noindent Taking the first and second derivatives of $f(r)$
\[
    \frac{df}{dr} = C e^{-r/a_0} \left( 1 - \frac{r}{a_0} \right)
\]
\[
    \frac{d^2f}{dr^2} = C e^{-r/a_0} \left( \frac{r}{a_0^2} - \frac{2}{a_0} \right)
\]
\noindent Substituting $\frac{d^2f}{dr^2}$ back into the Schrödinger equation
\[
    -\frac{\hbar^2}{2m} \left( \frac{r}{a_0^2} - \frac{2}{a_0} \right) - \frac{q^2}{4\pi\epsilon_0 r} (r) = E(r)
\]
\noindent Using the definition of $a_0$, the $1/r$ terms cancel, leaving the constant energy
\[
\boxed{
    E = -\frac{\hbar^2}{2ma_0^2} = \qty{-13.6}{\eV}
}
\]

\noindent The analytical result is close to the numerical, this confirms that the ground-state energy of Hydrogen is close to $\qty{-13.5}{\eV}$

% 2f
\subsection{Comparison with the Analytical Result}
\noindent Superimposing a plot of $a|f(r)|^2$, with $f(r)$ given by the analytical expression (\ref{eq:2e_analytical}), over the numerical plot produces the plot in Figure \ref{fig:2f_eigenvalue1}.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/2f_eigenvalue1.png}
    \caption{Position-probability densities from the numerical wave functions $\{\phi\}$ for eigenvalue number $1$ with the analytical result superimposed above it.}
    \label{fig:2f_eigenvalue1}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{imgs/2f_eigenvalue2.png}
    \caption{Position-probability densities from the numerical wave functions $\{\phi\}$ for eigenvalue number $2$.}
    \label{fig:2f_eigenvalue2}
\end{figure}

\noindent By observing Figure \ref{fig:2f_eigenvalue1}, it is clear that the analytical result closely matches the numerical. This is expected as if the eigenenergies ($E$) are approximately the same, the correspodning eigenfunctions ($f(r)$) must also be approximately the same.

\section{Momentum for the Particle in a Box}
%3a
\subsection{Change of Variables}
The ground-state ($n = 1$) wave function can be written as
\begin{equation}
	\Phi(x) = \sqrt{\frac{2}{L}} sin\left( \frac{\pi}{L} x \right)
\end{equation}
Implementing the change of variables $x' = x - L/2$ produces the ground-state wave function on the $x'$-axis:
\begin{align*}
    \Phi(x')
    &= \sqrt{\frac{2}{L}}\sin\left( \frac{\pi}{L}\left(x' + \frac{L}{2} \right)\right) \\
    &= \sqrt{\frac{2}{L}}\sin\left( \frac{\pi}{L}x' + \frac{\pi}{2} \right) \\
\end{align*}
\begin{equation}\label{eq:3a_groundstate}
\boxed{
    \Phi(x') = \sqrt{\frac{2}{L}}\cos\left( \frac{\pi x'}{L} \right) \\
}
\end{equation}

%3b
% (i)
\subsection{Solving for the Momentum}
\subsubsection{Fourier-Transform of the Wave Function}
The wave function for the ground-state (\ref{eq:3a_groundstate}) should really be written as
\begin{equation}
    \Phi(x') = \sqrt{\frac{2}{L}}\cos\left( \frac{\pi x'}{L} \right) \times \text{rect} \left( \frac{x'}{L} \right) \\
\end{equation}
Now taking the Fourier-transform of the ground-state wave function produces
\begin{align*}
    \FT \left[\phi(x')\right] &= \FT \left[\sqrt{\frac{2}{L}}\cos\left( \frac{\pi x'}{L} \right) \cdot \text{rect} \left( \frac{x'}{L} \right) \right] \\
    A(k') &= \sqrt{\frac{2}{L}} \times \FT \left[ \cos\left( \frac{\pi x'}{L} \right) \cdot \text{rect} \left( \frac{x'}{L} \right) \right] \\
A(k') &= \sqrt{\frac{2}{L}} \mathcal{F} \left[ \cos(k_1 x') \text{rect} \left( \frac{x'}{L} \right) \right] \quad \text{where } k_1 = \frac{\pi}{L} \\
&= \sqrt{\frac{2}{L}} \cdot \frac{1}{2} \left[ \mathcal{F}\left\{ \text{rect}\left(\frac{x'}{L}\right) \right\}_{k' \to (k'+k_1)} + \mathcal{F}\left\{ \text{rect}\left(\frac{x'}{L}\right) \right\}_{k' \to (k'-k_1)} \right] \\
\intertext{Using } \mathcal{F}\left[\text{rect}\left(\frac{t}{\tau}\right)\right] = \frac{\tau}{\sqrt{2\pi}}\mathrm{sinc}\left(\frac{\omega\tau}{2\pi}\right): \\
A(k') &= \sqrt{\frac{2}{L}} \cdot \frac{1}{2} \left[ \frac{L}{\sqrt{2\pi}} \mathrm{sinc}\left( \frac{L(k' + k_1)}{2\pi} \right) + \frac{L}{\sqrt{2\pi}} \mathrm{sinc}\left( \frac{L(k' - k_1)}{2\pi} \right) \right] \\
&= \frac{1}{2} \left( \sqrt{\frac{2}{L}} \cdot \frac{L}{\sqrt{2\pi}} \right) \left\{ \mathrm{sinc}\left[ \frac{L}{2\pi}(k' + k_1) \right] + \mathrm{sinc}\left[ \frac{L}{2\pi}(k' - k_1) \right] \right\} \\
&= \frac{1}{2} \sqrt{\frac{2L^2}{2\pi L}} \left\{ \mathrm{sinc}\left[ \frac{L}{2\pi}(k' + k_1) \right] + \mathrm{sinc}\left[ \frac{L}{2\pi}(k' - k_1) \right] \right\} \\
A(k') &= \frac{1}{2} \sqrt{\frac{L}{\pi}} \left\{ \mathrm{sinc}\left[ \frac{L}{2\pi}(k' + k_1) \right] + \mathrm{sinc}\left[ \frac{L}{2\pi}(k' - k_1) \right] \right\}
\end{align*}
\subsubsection{Wave Function in Momentum Space}
...
\begin{listing}[H]
    \caption{Code in 1d.py used to generate Figures \ref{fig:1d_probdensity} and \ref{fig:1d_numerical}.}
    \inputminted[
        bgcolor=monokaibg,
...
\begin{listing}[H]
    \caption{Code in 2c.py used to generate Figures \ref{fig:2f_eigenvalue1} and \ref{fig:2f_eigenvalue2}.}
    \inputminted[
        bgcolor=monokaibg,
        frame=leftline,        % Adds a subtle vertical bar on the left
        rulecolor=linegray,
